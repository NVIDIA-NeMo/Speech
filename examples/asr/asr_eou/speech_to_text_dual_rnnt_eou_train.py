...
from nemo.collections.asr.models.asr_eou_models import EncDecDualRNNTBPEEOUModel
...
def init_from_pretrained_nemo(model: EncDecDualRNNTBPEEOUModel, pretrained_model_path: str, cfg: DictConfig):
    """
    ...
    eou_decoder = model.eou_decoder  # type: RNNTDecoder
    ...
    eou_decoder.prediction["dec_rnn"].load_state_dict(
        pretrained_decoder.prediction["dec_rnn"].state_dict(), strict=True
    )
    ...
    eou_decoder.prediction["embed"].load_state_dict({"weight": decoder_embed_states}, strict=True)
    ...
    eou_joint_network = model.eou_joint  # type: RNNTJoint
    ...
    eou_joint_network.pred.load_state_dict(pretrained_joint_network.pred.state_dict(), strict=True)
    eou_joint_network.enc.load_state_dict(pretrained_joint_network.enc.state_dict(), strict=True)
    ...
    eou_joint_network.joint_net.load_state_dict(joint_state, strict=True)
    logging.info(f"Joint network weights loaded from {pretrained_model_path}.")


@hydra_runner(config_path="../conf/asr_eou", config_name="fastconformer_dual_transducer_bpe_streaming")
def main(cfg):
    ...
    asr_model = EncDecDualRNNTBPEEOUModel(cfg=cfg.model, trainer=trainer)
