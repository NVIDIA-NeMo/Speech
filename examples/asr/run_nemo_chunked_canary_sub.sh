#!/bin/bash
#
# NeMo Chunked AED ASR Evaluation Script for HuggingFace Datasets
# Uses speech_to_text_aed_chunked_infer.py for Canary models (AED architecture)
#
# Usage:
#   ./run_nemo_chunked_canary_sub.sh
#
...
CONFIG_NAME="${CONFIG_NAME:-canary_1b_v2.yaml}"
DATASET_PATH="${DATASET_PATH:-hf-audio/esb-datasets-test-only-sorted}"
BATCH_SIZE="${BATCH_SIZE:-1}"
CUDA_DEVICE="${CUDA_DEVICE:-0}"
OUTPUT_DIR="${OUTPUT_DIR:-./chunked_canary_results}"
...
echo "========================================"
echo "NeMo Chunked AED (Canary) ASR Evaluation"
echo "========================================"
...
run_eval() {
    ...
    # Run inference with AED chunked script
    python "${INFER_SCRIPT}" \
...
echo ""
echo "========================================"
echo "Chunked AED (Canary) Evaluation Complete!"
