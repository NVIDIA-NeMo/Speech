#!/bin/bash
#
# NeMo Chunked AED ASR Evaluation Script for HuggingFace Datasets (CPU Version)
# Uses speech_to_text_aed_chunked_infer.py for Canary models (AED architecture)
# Runs on CPU for performance measurement
#
# Usage:
#   ./run_nemo_chunked_canary_sub_cpu.sh
#
# Environment variables (optional):
#   CONFIG_NAME - Config file name (default: canary_1b_v2.yaml)
...
CONFIG_NAME="${CONFIG_NAME:-canary_1b_v2_streaming_cpu.yaml}"
DATASET_PATH="${DATASET_PATH:-hf-audio/esb-datasets-test-only-sorted}"
BATCH_SIZE="${BATCH_SIZE:-1}"
OUTPUT_DIR="${OUTPUT_DIR:-./chunked_canary_cpu_results}"
...
INFER_SCRIPT="${SCRIPT_DIR}/asr_chunked_inference/aed/speech_to_text_aed_chunked_infer.py"
CONFIG_PATH="${SCRIPT_DIR}/conf/asr_streaming_inference/"

echo "========================================"
echo "NeMo Chunked AED (Canary) ASR Evaluation (CPU)"
echo "========================================"
...
run_eval() {
    ...
    # Run inference with AED chunked script on CPU (cuda=-1 forces CPU)
    python "${INFER_SCRIPT}" \
...
echo ""
echo "========================================"
echo "Chunked AED (Canary) CPU Evaluation Complete!"
