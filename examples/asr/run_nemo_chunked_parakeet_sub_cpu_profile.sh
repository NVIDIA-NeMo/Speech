#!/bin/bash
#
# NeMo Chunked/Buffered ASR Evaluation Script for HuggingFace Datasets - CPU Version
# Uses asr_streaming_infer.py with Hydra config for buffered streaming
#
# Usage:
#   ./run_nemo_chunked_parakeet_sub_cpu.sh
#
# Environment variables (optional):
#   CONFIG_NAME - Config file name (default: parakeet_tdt_0.6b_v3_cpu.yaml)
...
CONFIG_NAME="${CONFIG_NAME:-parakeet_tdt_0.6b_v3_cpu.yaml}"
DATASET_PATH="${DATASET_PATH:-hf-audio/esb-datasets-test-only-sorted}"
BATCH_SIZE="${BATCH_SIZE:-1}"
OUTPUT_DIR="${OUTPUT_DIR:-./chunked_results_cpu}"
...
echo "========================================"
echo "NeMo Chunked/Buffered ASR Evaluation (CPU)"
echo "========================================"
...
run_eval() {
    ...
    # Run inference on CPU (no CUDA device override needed)
    python "${INFER_SCRIPT}" \
...
echo ""
echo "========================================"
echo "Chunked/Buffered Evaluation Complete! (CPU)"
