#!/bin/bash
#
# NeMo Chunked/Buffered ASR Evaluation Script for stt_en_conformer_transducer_xlarge (CPU Version)
# Uses asr_streaming_infer.py with buffered streaming for ASR leaderboard evaluation
# Runs on CPU for performance measurement
#
# Model: https://huggingface.co/nvidia/stt_en_conformer_transducer_xlarge
#
# Usage:
#   ./run_nemo_conformer_transducer_xlarge_sub_cpu.sh
#
# Environment variables (optional):
#   BATCH_SIZE - Batch size for evaluation (default: 1)
#   NUM_THREADS - Number of CPU threads (default: number of CPUs)
#   OUTPUT_DIR - Output directory for results

set -e  # Exit on error

# Configuration
CONFIG_NAME="${CONFIG_NAME:-conformer_transducer_xlarge_cpu.yaml}"
DATASET_PATH="${DATASET_PATH:-hf-audio/esb-datasets-test-only-sorted}"
BATCH_SIZE="${BATCH_SIZE:-1}"
OUTPUT_DIR="${OUTPUT_DIR:-./conformer_transducer_xlarge_cpu_results}"
MAX_EVAL_SAMPLES="${MAX_EVAL_SAMPLES:-3}"
AUDIO_CACHE_DIR="${AUDIO_CACHE_DIR:-${OUTPUT_DIR}/manifests}"

# CPU settings
NUM_THREADS="${NUM_THREADS:-$(nproc)}"
# Cap threads at 64 for NUMEXPR compatibility
if [ "${NUM_THREADS}" -gt 64 ]; then
    NUM_THREADS=64
fi
...
INFER_SCRIPT="${SCRIPT_DIR}/asr_streaming_inference/asr_streaming_infer_profile.py"
CONFIG_PATH="${SCRIPT_DIR}/conf/asr_streaming_inference/"

echo "========================================"
echo "NeMo Buffered Streaming ASR Evaluation (CPU)"
...
run_eval() {
    ...
    local log_file="${OUTPUT_DIR}/log_${output_name}_profile.txt"
    
    echo ""
    echo "========================================"
    echo "Profiling on ${dataset}/${split}..."
    echo "========================================"
    
    # Create manifest (reuses cached if available)
    manifest_file=$(create_manifest "${dataset}" "${split}")
    
    # Create a trimmed manifest with only MAX_EVAL_SAMPLES entries for profiling
    local profile_manifest="${OUTPUT_DIR}/profile_manifest_${output_name}.json"
    head -n ${MAX_EVAL_SAMPLES} "${manifest_file}" > "${profile_manifest}"
    echo "Using ${MAX_EVAL_SAMPLES} samples from manifest for profiling"
    ...
    python "${INFER_SCRIPT}" \
        ...
        audio_file="${profile_manifest}" \
        output_filename="${OUTPUT_DIR}/output_${output_name}_profile.json" \
...
print('')
print('=' * 50)
print(f'  {dataset}/{split} RESULTS (CPU)')
print('=' * 50)
...
echo ""
python "${SCRIPT_DIR}/calculate_average_wer.py" "${OUTPUT_DIR}" --model_name "stt_en_conformer_transducer_xlarge_cpu"
