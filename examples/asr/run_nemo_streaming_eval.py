...
# Check for --cpu flag early to disable CUDA before imports
...
import torch
# Disable mkldnn/oneDNN for CPU mode to avoid LSTM primitive errors
if '--cpu' in os.sys.argv:
...
def evaluate_streaming_model(args):
    ...
    # Determine total samples for progress tracking
    if args.max_eval_samples > 0:
        total_samples = args.max_eval_samples
    else:
        # For non-streaming datasets, get the length
    ...
    for sample_idx, sample in enumerate(tqdm(dataset, desc=f"Evaluating {args.dataset}")):
        ...
        if (sample_idx + 1) % batch_size == 0 or is_last_sample:
            with torch.amp.autocast('cuda' if device.type == "cuda" else "cpu", dtype=amp_dtype, enabled=args.amp):
