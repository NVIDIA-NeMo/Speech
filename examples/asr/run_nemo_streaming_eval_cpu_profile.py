...
# Mock nv_one_logger - use separate MagicMock per submodule
for _mod in [
    'nv_one_logger',
    'nv_one_logger.api',
...
# Patch one_logger_callback module to avoid metaclass conflict
import types as _types
_fake_olc = _types.ModuleType('nemo.lightning.one_logger_callback')
_fake_olc.OneLoggerNeMoCallback = object
sys.modules['nemo.lightning.one_logger_callback'] = _fake_olc
...
def perform_streaming(
    ...
    with record_function("get_initial_cache_state"):
    ...
    for step_num, (chunk_audio, chunk_lengths) in enumerate(streaming_buffer_iter):
        with torch.inference_mode():
            ...
            with torch.no_grad():
                with record_function(f"conformer_stream_step_{step_num}"):
                    (
                        pred_out_stream,
                        transcribed_texts,
                        cache_last_channel,
    ...
    if args.manifest_file:
        with profile(
            ...
            for sample_idx, entry in enumerate(tqdm(sample_iter, total=total_samples, desc=eval_desc)):
                ...
                with record_function("append_audio_file"):
                ...
                if (sample_idx + 1) % batch_size == 0 or is_last_sample:
                    with torch.no_grad():
                        with record_function("perform_streaming_batch"):
        ...
        print("\n" + "=" * 80)
        print("PYTORCH PROFILER RESULTS (sorted by CPU time)")
        print("=" * 80)
        print(prof.key_averages().table(sort_by="cpu_time_total", row_limit=30))
        print("\n" + "=" * 80)
        print("PYTORCH PROFILER RESULTS (sorted by self CPU time)")
        print("=" * 80)
        ...
        logging.info(f"Profiler trace saved to: {trace_path} (open in chrome://tracing)")
    else:
