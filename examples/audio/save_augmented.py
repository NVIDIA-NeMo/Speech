...
def main(cfg: DictConfig):
    assert cfg.get("input_cuts", None) is not None, "input_cuts is required, please override (for example, +input_cuts=some_path/cuts.jsonl)"
    assert cfg.get("output_cuts", None) is not None, "output_cuts is required, please override (for example, +output_cuts=some_path/cuts.augmented.jsonl)"
    ...
    dataloader = get_lhotse_dataloader_from_config(
        OmegaConf.create(cfg.model.train_ds),
        global_rank=0,
        world_size=1,
    ...
    with CutSet.open_writer(output_cuts_path) as writer:
        for i, (sample, original_cut) in enumerate(tqdm(zip(islice(dataloader, num_samples), cuts), total=num_samples)):
            ...
            cut = MonoCut(id=dirty_recording.id, start=0, channel=0, duration=dirty_recording.duration, recording=dirty_recording)
            cut.target_recording = clean_recording

            for optional_field_name in ('text', 'original_text', 'language',):
                if hasattr(original_cut, optional_field_name) and getattr(original_cut, optional_field_name) is not None:
