...
def collate_and_tokenize_custom(
    batch, model, extra_duration_thrshould=1.3, sample_rate=22050, root_path=None, drop_BOS=False
):
    tokenized_list = []
        ...
        if isinstance(text_data, list):
            ...
            if drop_BOS:
                tokenized_list.append(
                    torch.as_tensor(model.tokenizer.text_to_ids(text_data), dtype=torch.long, device=model.device)
                )
            else:
                tokenized_list.append(
                    torch.as_tensor(
...
def inference(cfg):
    ...
    for batch_id, batch in enumerate(read_jsonl_batches(cfg.datasets_json_path, cfg.batch_size, max_batches=None)):
        inputs = collate_and_tokenize_custom(
            ...
            drop_BOS=cfg.get("drop_BOS", False),
        )
