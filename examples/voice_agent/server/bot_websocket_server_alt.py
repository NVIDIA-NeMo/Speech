...
"""
Alternative voice agent server that runs on port 8766 (instead of 8765).
This is useful for testing agent-to-agent communication.

Usage:
    python bot_websocket_server_alt.py
    
Or with a custom port:
    python bot_websocket_server_alt.py --port 8767
    
Or with a custom config:
    SERVER_CONFIG_PATH=server_configs/custom_config.yaml python bot_websocket_server_alt.py
"""
...
def setup_logging(log_file="bot_server_alt.log"):
    ...
    logger.add(log_file, rotation="1 day", level="DEBUG")
...
async def run_bot_websocket_server(port=8766, host="0.0.0.0"):
    ...
    setup_logging(log_file=f"bot_server_alt_{port}.log")
...
def main():
    parser = argparse.ArgumentParser(description="Alternative voice agent server (runs on port 8766 by default)")
    parser.add_argument(
        "--port",
        type=int,
        default=8766,
    ...
    setup_logging(log_file=f"bot_server_alt_{args.port}.log")

    logger.info("=" * 80)
    logger.info(f"Alternative Voice Agent Server - Port {args.port}")
    logger.info("=" * 80)

    asyncio.run(run_bot_websocket_server(port=args.port, host=args.host))
