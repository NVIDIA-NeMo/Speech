...
from nemo.collections.asr.parts.submodules.asr_transformer_modules import ASRTransformerLayer
...
__all__ = ['ASRTransformerEncoder']


class ASRTransformerEncoder(NeuralModule, StreamingEncoder, Exportable, AccessMixin):
    """
    ASR encoder based on Transformer.

    Args:
        feat_in (int): the size of feature channels
        n_layers (int): number of layers of ASRTransformerLayer
...
class ASRTransformerEncoderAdapter(ASRTransformerEncoder, adapter_mixins.AdapterModuleMixin):

    # Higher level forwarding
    def add_adapter(self, name: str, cfg: dict):
        cfg = self._update_adapter_cfg_input_dim(cfg)
        for layer in self.layers:  # type: adapter_mixins.AdapterModuleMixin
            layer.add_adapter(name, cfg)

    def is_adapter_available(self) -> bool:
        return any([layer.is_adapter_available() for layer in self.layers])

    def set_enabled_adapters(self, name: Optional[str] = None, enabled: bool = True):
        for layer in self.layers:  # type: adapter_mixins.AdapterModuleMixin
            layer.set_enabled_adapters(name=name, enabled=enabled)

    def get_enabled_adapters(self) -> List[str]:
        names = set([])
        for layer in self.layers:  # type: adapter_mixins.AdapterModuleMixin
            names.update(layer.get_enabled_adapters())
...
class ASRTransformerFeatureExtractor(NeuralModule, Exportable, AccessMixin):
    """
    A wrapper module that extracts features from multiple layers of a ASRTransformerEncoder,
    ...
    def __init__(
        ...
        if layer_idx_list is None:
            self.layer_idx_list = [i for i in range(len(encoder.layers))]
        else:
...
"""
if adapter_mixins.get_registered_adapter(ASRTransformerEncoder) is None:
    adapter_mixins.register_adapter(base_class=ASRTransformerEncoder, adapter_class=ASRTransformerEncoderAdapter)


@dataclass
class ASRTransformerChangeConfig:
