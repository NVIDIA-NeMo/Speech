...
:class:`EnglishAcronymNormalizer` (``"b b c"`` -> ``"bbc"``), :class:`EnglishNameNormalizer`, and a
table of multi-word compound rewrites (``"wi fi"`` -> ``"wifi"``). Scores produced with
``whisper_normalizer`` are therefore not comparable to leaderboard numbers -- use this module when
reproducing Open ASR Leaderboard WER.

Deviations from upstream (behaviour-preserving):

* ``import regex`` is deferred into :meth:`BasicTextNormalizer.__call__` and ``import num2words``
  into :meth:`MultilingualNormalizer._normalize_numbers`, so neither becomes a hard NeMo
  dependency -- they are only needed for ``split_letters=True`` and non-English number
...
Example::
    ...
    normalizer = get_hf_normalizer("en")
    normalizer("Mr. Smith paid $1,000 for the Wi-Fi.")  # 'mister smith paid $1000 for the wifi'
"""
...
# --------------------------------------------------------------------------------------------------------------------
# Normalizer classes, from open_asr_leaderboard/normalizer/normalizer.py
# --------------------------------------------------------------------------------------------------------------------
...
class MultilingualNormalizer(BasicMultilingualTextNormalizer):
    ...
    def _normalize_numbers(self, text, lang):
        # Deferred import: only needed for non-English, so it is not a hard NeMo dependency.
        try:
            import num2words
        except ImportError as e:
            raise ImportError(
                f"num2words is required to normalize numbers for language '{lang}'. Install it with "
                "`pip install num2words`, or use the English normalizer (language='en'), which does "
                "not need it."
            ) from e
...
def get_hf_normalizer(language: str = "en"):
    """Build the Open ASR Leaderboard normalizer for ``language``.

    Mirrors the dispatch in ``open_asr_leaderboard/normalizer/eval_utils.py::score_results``:
    English uses :class:`EnglishTextNormalizer`, anything else uses :class:`MultilingualNormalizer`
    with ``remove_diacritics=False`` and digits spelled out for that language via ``num2words``.

    Args:
        language: BCP-47-ish language code, e.g. ``"en"``, ``"de"``, ``"fr"``.

    Returns:
        A callable mapping a raw string to its normalized form.
    """
    if language == "en":
        return EnglishTextNormalizer()

    ml_normalizer = MultilingualNormalizer(remove_diacritics=False)
    return lambda text: ml_normalizer(text, lang=language)
