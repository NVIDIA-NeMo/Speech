...
from nemo.collections.llm.gpt.model.llama import Llama2Config13B, LlamaModel
...
def model() -> run.Config[pl.LightningModule]:
    """
    Factory function to create a Llama2 13B model configuration.

    Returns:
        run.Config[pl.LightningModule]: Configuration for the Llama2 13B model.

    Examples:
        CLI usage:
            $ nemo llm pretrain model=llama2_13b ...

        Python API usage:
            >>> model_config = model()
            >>> print(model_config)
    """
    return run.Config(LlamaModel, config=run.Config(Llama2Config13B))
...
def trainer(
    ...
    """
    Configure the NeMo Lightning Trainer for Llama2 13B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm pretrain trainer=llama2_13b ...
...
def pretrain_recipe(
    ...
    """
    Create a pre-training recipe for Llama2 13B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm pretrain --factory llama2_13b
            $ nemo llm pretrain --factory "llama2_13b(num_nodes=2, name='my_pretrain')"

        Python API usage:
            >>> recipe = pretrain_recipe(name="llama2_13b_pretrain", num_nodes=2)
            >>> print(recipe)
    """
    recipe = run.Partial(
        fn,
...
def pretrain_performance_optimizations(recipe: run.Partial) -> run.Partial:
    """
    Create a performance-optimized pre-training recipe for Llama2 13B model.
...
def finetune_recipe(
    ...
    """
    Create a fine-tuning recipe for Llama2 13B model.
    ...
    Examples:
        CLI usage:
            $ nemo llm finetune --factory llama2_13b

        Python API usage:
            >>> recipe = finetune_recipe(name="llama2_13b_finetune", num_nodes=2)
            >>> print(recipe)
    ...
    recipe = default_finetune_recipe(
        model(), "meta-llama/Llama-2-13b-hf", dir, name, num_nodes, num_gpus_per_node, packed_sequence
    )
