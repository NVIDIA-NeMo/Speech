...
def _is_gemma4(llm) -> bool:
    """Detect whether the loaded LLM is a Gemma 4 multimodal model.

    Gemma 4 wraps its language model one level deeper:
        llm.model.language_model   (Gemma4)
    vs. the standard layout:
        llm.model                  (everyone else)
    """
    return hasattr(llm, "model") and hasattr(llm.model, "language_model")

def _setup_normal(salm: "SALM") -> None:
    """Move embed_tokens out of llm.model to avoid FSDP/TP hook issues."""
    salm.embed_tokens = salm.llm.model.embed_tokens
    del salm.llm.model.embed_tokens

def _setup_gemma4(salm: "SALM") -> None:
    """Apply all patches required for Gemma 4's nested multimodal architecture.

    Gemma 4 buries embed_tokens one level deeper than standard HF models, adds
    multimodal placeholder routing, and requires per_layer_inputs in several
    internal call sites.  These patches make SALM's audio-injection path work
    without touching Gemma's own forward logic.
    """
    lm = salm.llm.model.language_model  # shorthand

    # ------------------------------------------------------------------
    # Patch 1 — pull embed_tokens out to avoid duplicate parameters while
    # keeping Gemma's own forward pass intact via pre/post hooks.
    # ------------------------------------------------------------------
    salm.embed_tokens = lm.embed_tokens
    del lm.embed_tokens

    def _pre_forward_hook(module, args, kwargs):
        module.model.language_model.embed_tokens = salm.embed_tokens
        return args, kwargs

    def _post_forward_hook(module, args, output):
        # Remove so PyTorch doesn't see the parameter twice.
        del module.model.language_model.embed_tokens

    salm.llm.register_forward_pre_hook(_pre_forward_hook, with_kwargs=True)
    salm.llm.register_forward_hook(_post_forward_hook)

    # Redirect get/set_input_embeddings on all three relevant modules so that
    # HF internals (e.g. resize_token_embeddings) always resolve to our copy.
    def _get_embeddings(inner_self):
        return salm.embed_tokens

    def _set_embeddings(inner_self, value):
        salm.embed_tokens = value

    for module in (salm.llm, salm.llm.model, lm):
    ...
    # ------------------------------------------------------------------
    # Patch 2 — disable Gemma 4's multimodal placeholder routing.
    # SALM injects audio embeddings directly, so the vision/audio slot
    # detection logic must be a no-op.
    # ------------------------------------------------------------------
    ...
    salm.llm.model.get_placeholder_mask = types.MethodType(
        _noop_placeholder_mask, salm.llm.model
    )
    ...
    # ------------------------------------------------------------------
    # Patch 4 — ensure language_model.forward always receives
    # per_layer_inputs (defaults to None when not supplied).
    # ------------------------------------------------------------------
    ...
    lm.forward = types.MethodType(_patched_lm_forward, lm)

class SALM(LightningModule, HFHubMixin):
    """Speech-Augmented Language Model.

    Supports both standard HF language models and Gemma 4's nested multimodal
    architecture.  Model-specific setup is handled transparently at init time.
    """
    ...
    def __init__(self, cfg) -> None:
        ...
        # We detect Gemma 4 by the presence of llm.model.language_model.
        # All other models take the original code path.
        if _is_gemma4(self.llm):
            _setup_gemma4(self)
        else:
            _setup_normal(self)
    ...
    # -----------------------------------------------------------------------
    # embed_tokens property
    # -----------------------------------------------------------------------
    # Both code paths store embed_tokens as a plain attribute on `self`.
    # The property below is intentionally NOT defined so that attribute access
    # resolves directly to `self.__dict__["embed_tokens"]`, matching the
    # original behaviour for normal models and keeping Gemma4's hooks simple.
    #
    # If you ever need to expose embed_tokens as a @property (e.g. to always
    # read from lm.embed_tokens after Gemma4 hooks restore it), uncomment:
    #
    # @property
