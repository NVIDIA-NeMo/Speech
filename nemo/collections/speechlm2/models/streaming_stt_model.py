...
class StreamingSTTModelConfig:
    ...
    # Only effective when ``compact_template=False`` — in compact mode the
    # write token already wraps every assistant turn unconditionally.
    # Requires ``blank_token != ""`` (validated at startup).
...
class StreamingSTTModel(LightningModule, HFHubMixin):
    ...
    def __init__(
        ...
        # Write token registration: needed for compact_template (which wraps every
        # assistant turn) AND for prepend_write_token (which prepends to non-empty
        # assistant content for the binary content-gate). Default <|im_start|> is
        # already in Qwen3's vocab → uses pretrained embedding.
        if self.core_cfg.compact_template or self.core_cfg.prepend_write_token:
        ...
        if self.core_cfg.prepend_write_token:
            ...
            if self.core_cfg.compact_template:
                logging.warning(
                    "prepend_write_token=True has no effect when compact_template=True "
                    "(the compact path already wraps every assistant turn in write_token)."
                )
    ...
    def content_score_mode(self) -> Optional[str]:
        ...
        Returns one of:
            ...
            - "binary": fixed + non-compact + prepend_write_token=True +
              blank_token != "". Calibrated p_write / (p_write + p_blank).
        ...
        **generation_kwargs,
        ...
        if self.content_score_mode is not None:
            wt = (
                ...
                if (self.core_cfg.compact_template or getattr(self.core_cfg, "prepend_write_token", False))
                else None
            )
