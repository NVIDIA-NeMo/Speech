...
from nemo.collections.asr.parts.mixins.streaming import DummyCacheAwareStreamingMixin, StreamingEncoder
...
class AudioPerceptionModule(NeuralModule, Exportable):
    ...
    @property
    def supports_cache_aware_streaming(self) -> bool:
        """Whether the mounted encoder carries real cache-aware streaming state.

        Encoders implementing the full :class:`StreamingEncoder` protocol (e.g. ``ConformerEncoder``,
        ``StreamingTransformerEncoder``) are driven through ``cache_aware_stream_step`` when
        ``forward(..., streaming=True)`` is used. Stateless encoders that only opt into the interface via
        :class:`DummyCacheAwareStreamingMixin` (e.g. ``SubsamplingEncoder``) are excluded: they have no KV
        cache, so they run the regular forward and report a ``None`` cache to the caller.
        """
        encoder = self.encoder
        return isinstance(encoder, StreamingEncoder) and not isinstance(encoder, DummyCacheAwareStreamingMixin)
    ...
    def forward(
        ...
        elif streaming and self.supports_cache_aware_streaming:
            # `spk_targets` must reach the STREAMING call too, not just the offline one below.
            # Oracle speaker targets are consumed by both streaming decoders, and a mounted
            # ParallelExpertEncoder uses them in place of running its own diarizer -- dropping
            # them here would silently fall back to the diarizer while the caller believed it had
            # supplied oracle attribution.
            stream_kwargs = {}
    ...
    def get_initial_cache_state(self, batch_size=1, dtype=torch.float32, device=None, **kwargs):
        """Allocate the encoder-side streaming cache consumed by ``forward(..., streaming=True)``.

        Returns the ``(cache_last_channel, cache_last_time, cache_last_channel_len)`` triple produced by the
        mounted encoder. Stateless encoders using :class:`DummyCacheAwareStreamingMixin` return
        ``(None, None, None)``; callers are expected to skip cache bookkeeping in that case.
        """
        if isinstance(self.modality_adapter, (QformerConnector, MultiLayerProjectionConnector)):
