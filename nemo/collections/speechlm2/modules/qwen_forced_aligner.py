...
class QwenForcedAligner:
    ...
    def __init__(
        ...
        self.aligner = qwen_asr.Qwen3ForcedAligner.from_pretrained(
            pretrained_model, torch_dtype=dtype, device_map=device,
        )
    ...
    def align_numpy(
        ...
        for input_id, output_id, word_list in zip(
            ...
            timestamp_ms = (
                (masked_output_id * self.aligner.timestamp_segment_time).cpu().float().numpy()
            )
            ts_out = self.aligner.aligner_processor.parse_timestamp(
    ...
    def align(
        ...
        audio_list = [
            (audio_cpu[i, : audio_lens[i]].numpy(), self.SAMPLE_RATE)
