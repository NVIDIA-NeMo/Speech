...
class EasyMagpieTTSInferenceModel(ModelPT):
    ...
    def __init__(self, cfg: DictConfig, trainer: 'Trainer' = None):
        ...
        elif self.decoder_type == 'trt_backbone':
            # TRT-LLM accelerated backbone: inputs_embeds → last_hidden_state
            # ~2.5× faster than HF Qwen2Model (11.7 ms vs 29.4 ms per decode step)
            from nemo.collections.tts.models.trt_backbone_runner import TRTBackboneDecoder
            engine_dir = cfg.get('trt_backbone_engine_dir')
            assert engine_dir is not None, "decoder_type=trt_backbone requires cfg.trt_backbone_engine_dir"
            trt_device = torch.device(cfg.get('trt_backbone_device', 'cuda:0'))
            self.decoder = TRTBackboneDecoder(engine_dir=engine_dir, device=trt_device)
            self.lm_text_head = None
            logging.info(f"TRT backbone loaded from {engine_dir} on {trt_device}")

        else:
            raise ValueError(f"Unknown decoder_type: {self.decoder_type}. Supported: 'huggingface', 'nemotron_h', 'trt_backbone'")
