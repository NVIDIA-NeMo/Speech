...
def _is_cuda_python_12_8_or_newer():
    """Check if cuda-python version is 12.8.0 or newer (uses new API signatures)."""
    from cuda.bindings import __version__ as cuda_python_version

    return Version(cuda_python_version) >= Version("12.8.0")


# Cache the version check result for performance
_CUDA_PYTHON_12_8_OR_NEWER = None


def _get_cuda_python_12_8_or_newer():
    """Get cached result of CUDA Python version check."""
    global _CUDA_PYTHON_12_8_OR_NEWER
    if _CUDA_PYTHON_12_8_OR_NEWER is None:
        _CUDA_PYTHON_12_8_OR_NEWER = _is_cuda_python_12_8_or_newer()
    return _CUDA_PYTHON_12_8_OR_NEWER
...
def with_conditional_node(while_loop_kernel, while_loop_args, while_loop_conditional_handle, device):
    ...
    cuda_12_8_plus_api = _get_cuda_python_12_8_or_newer()
    ...
    # CUDA 12.8+ changed cuGraphAddNode signature from 5 args to 4 args (removed edge data parameter)
    if cuda_12_8_plus_api:
    ...
    # CUDA 12.8+ changed cudaStreamUpdateCaptureDependencies from 5 args to 4 (removed edge data)
    if cuda_12_8_plus_api:
        cu_call(
    ...
    # CUDA 12.8+ changed cudaStreamBeginCaptureToGraph from 6 args to 4 (removed edge data params)
    if cuda_12_8_plus_api:
        cu_call(
