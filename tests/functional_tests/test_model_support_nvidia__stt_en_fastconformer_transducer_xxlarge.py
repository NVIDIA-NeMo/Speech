...
def test_model_inference():
    ...
    assert torch.is_tensor(encoded), "encoder output must be a tensor"
    assert encoded.ndim == 3, "encoder output must be 3-D (batch, hidden, time)"
    assert encoded.shape[0] == 1, "batch dimension must match input"
    assert torch.is_tensor(encoded_len), "encoder length must be a tensor"
    assert encoded_len.shape == (1,), "encoder length shape must be (batch,)"
